...
//    public Vector2(double x, double y, Int64 z = 0)
//    {
//      this.X = x; this.Y = y; this.Z = z;
//    }
    
//    public Vector2(Vector2 pt)
//    {
...
//}
  ...
  {    
    ...
    internal bool PointOnLineSegment(Vector2 pt, 
        Vector2 linePt1, Vector2 linePt2)
    {
    ...
    internal bool PointOnPolygon(Vector2 pt, OutPt pp)
    {
      OutPt pp2 = pp;
      while (true)
      {
    ...
    protected static bool SlopesEqual(Vector2 pt1, Vector2 pt2, Vector2 pt3)
    {
    ...
    protected static bool SlopesEqual(Vector2 pt1, Vector2 pt2, Vector2 pt3, Vector2 pt4)
    {
    ...
    public static Rectangle GetBounds(Paths paths)
    {
      int i = 0, cnt = paths.Count;
      while (i < cnt && paths[i].Count == 0) i++;
      if (i == cnt) return default(Rectangle);

        float top = paths[i][0].Y;
        float bottom = top;
        float left = paths[i][0].X;
        float right = left;
        for (; i < cnt; i++)
        {
        ...
        return Rectangle.FromTopBottomLeftRight(top, bottom, left, right);
  ...
  public class Clipper : ClipperBase
  {
...
#if use_xyz
      public delegate void TZFillCallback(Vector2 vert1, Vector2 vert2, ref Vector2 intersectPt);
...
#if use_xyz
      internal void SetZ(ref Vector2 pt, TEdge e)
      {
        pt.Z = 0;
      ...
      private bool HorzSegmentsOverlap(
        Vector2 Pt1a, Vector2 Pt1b, Vector2 Pt2a, Vector2 Pt2b)
      {
      ...
      private void IntersectEdges(TEdge e1, TEdge e2, Vector2 pt, bool protect = false)
      {
  ...
  //assumes that e1 will be to the Right of e2 ABOVE the intersection
      ...
      private bool IntersectPoint(TEdge edge1, TEdge edge2, out Vector2 ip)
      {
        ip = new Vector2();
        double b1, b2;
      ...
      private int PointInPolygon(Vector2 pt, OutPt op)
      {
  ...
  public class ClipperOffset
  {
    ...
    internal static Vector2 GetUnitNormal(Vector2 pt1, Vector2 pt2)
    {
      ...
      float f = 1 / MathHelper.Sqrt(dx * dx + dy * dy);
      dx *= f;
      dy *= f;
    ...
    private void DoOffset(double delta)
    {
      ...
      for (int i = 0; i < m_polyNodes.ChildCount; i++)
      {
        ...
        else
        {
          ...
          {
            int j = len - 1;
            pt1 = new Vector2((float)(m_srcPoly[j].X + m_normals[j].X *
          ...
          if (node.m_endtype == EndType.etOpenButt)
          {
            pt1 = new Vector2((float)(m_srcPoly[0].X - m_normals[0].X * delta),
    ...
    public void Execute(ref Paths solution, double delta)
    {
      ...
      {
        Rectangle r = Clipper.GetBounds(m_destPolys);
    ...
    void OffsetPoint(int j, ref int k, JoinType jointype)
    {
      ...
      if (m_sinA * m_delta < 0)
      {
        m_destPoly.Add(new Vector2((float)(m_srcPoly[j].X + m_normals[k].X * m_delta),
          (float)(m_srcPoly[j].Y + m_normals[k].Y * m_delta)));
        m_destPoly.Add(m_srcPoly[j]);
        m_destPoly.Add(new Vector2((float)(m_srcPoly[j].X + m_normals[j].X * m_delta),
    ...
    {
      double q = m_delta / r;
      m_destPoly.Add(new Vector2((float)(m_srcPoly[j].X + (m_normals[k].X + m_normals[j].X) * q),
    ...
    internal void DoRound(int j, int k)
    {
      ...
      {
        m_destPoly.Add(new Vector2((float)(m_srcPoly[j].X + X * m_delta),
