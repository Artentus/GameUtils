...
namespace ClipperLib
{
  ...
  using Path = List<ControlPoint>;
  using Paths = List<List<ControlPoint>>;
  ...
  public class PolyTree : PolyNode
  {
      ...
      PolyNode CloneNode(PolyNode node, PolyNode parent, List<PolyNode> allNodes)
      {
          var result = new PolyNode();
          result.m_Parent = parent;
          result.m_Index = node.m_Index;
          result.m_Childs = node.m_Childs.Select(child => CloneNode(child, result, allNodes)).ToList();
          result.m_endtype = node.m_endtype;
          result.m_jointype = node.m_jointype;
          result.m_polygon = new Path(node.m_polygon);
          result.IsOpen = node.IsOpen;
          allNodes.Add(result);
          return result;
      }
      ...
      public PolyTree Clone()
      {
          ...
          result.m_Childs = this.Childs.Select(child => CloneNode(child, this, result.m_AllPolys)).ToList();
          return result;
      }
  }
...
//    public ControlPoint(double x, double y, Int64 z = 0)
//    {
...
//      {
//        ControlPoint a = (ControlPoint)obj;
//        return (this == a);
//      }
//      else return false;
//    }
...
//}
  ...
  internal class TEdge {
    internal ControlPoint Bot;
    internal ControlPoint Curr;
    internal ControlPoint Top;
    internal ControlPoint Delta;
    internal double Dx;
  ...
  {    
    ...
    internal bool PointIsVertex(ControlPoint pt, OutPt pp)
    {
      OutPt pp2 = pp;
      do
      {
    ...
    internal bool PointOnLineSegment(ControlPoint pt, 
        ControlPoint linePt1, ControlPoint linePt2)
    {
    ...
    internal bool PointOnPolygon(ControlPoint pt, OutPt pp)
    {
      OutPt pp2 = pp;
      while (true)
      {
    ...
    protected static bool SlopesEqual(ControlPoint pt1, ControlPoint pt2, ControlPoint pt3)
    {
    ...
    protected static bool SlopesEqual(ControlPoint pt1, ControlPoint pt2, ControlPoint pt3, ControlPoint pt4)
    {
    ...
    internal bool Pt2IsBetweenPt1AndPt3(ControlPoint pt1, ControlPoint pt2, ControlPoint pt3)
    {
    ...
    public static RectangleF GetBounds(Paths paths)
    {
      ...
      if (i == cnt) return default(Rectangle);

        double top = paths[i][0].Y;
        double bottom = top;
        double left = paths[i][0].X;
        double right = left;
        for (; i < cnt; i++)
        {
            for (int j = 0; j < paths[i].Count; j++)
            {
                if (paths[i][j].X < left) left = paths[i][j].X;
                else if (paths[i][j].X > right) right = paths[i][j].X;
                if (paths[i][j].Y < top) top = paths[i][j].Y;
                else if (paths[i][j].Y > bottom) bottom = paths[i][j].Y;
            }
        }
  ...
  public class Clipper : ClipperBase
  {
...
#if use_xyz
      public delegate void TZFillCallback(ControlPoint vert1, ControlPoint vert2, ref ControlPoint intersectPt);
      ...
      private void AddJoin(OutPt Op1, OutPt Op2, ControlPoint OffPt)
      {
        Join j = new Join();
        j.OutPt1 = Op1;
        j.OutPt2 = Op2;
      ...
      private void AddGhostJoin(OutPt Op, ControlPoint OffPt)
      {
        Join j = new Join();
        j.OutPt1 = Op;
        j.OffPt = OffPt;
...
#if use_xyz
      internal void SetZ(ref ControlPoint pt, TEdge e)
      {
        pt.Z = 0;
      ...
      private void AddLocalMaxPoly(TEdge e1, TEdge e2, ControlPoint pt)
      {
          AddOutPt(e1, pt);
      ...
      private OutPt AddLocalMinPoly(TEdge e1, TEdge e2, ControlPoint pt)
      {
        OutPt result;
        TEdge e, prevE;
      ...
      private OutPt AddOutPt(TEdge e, ControlPoint pt)
      {
      ...
      internal void SwapPoints(ref ControlPoint pt1, ref ControlPoint pt2)
      {
          ControlPoint tmp = pt1;
          pt1 = pt2;
          pt2 = tmp;
      }
      //------------------------------------------------------------------------------
      ...
      private bool HorzSegmentsOverlap(
        ControlPoint Pt1a, ControlPoint Pt1b, ControlPoint Pt2a, ControlPoint Pt2b)
      {
      ...
      private OutPt InsertPolyPtBetween(OutPt p1, OutPt p2, ControlPoint pt)
      {
      ...
      private double GetDx(ControlPoint pt1, ControlPoint pt2)
      {
      ...
      private void IntersectEdges(TEdge e1, TEdge e2, ControlPoint pt, bool protect = false)
      {
  ...
  //assumes that e1 will be to the Right of e2 ABOVE the intersection
        ...
        for (;;)
        {
          ...
          while(e != null)
          {
              ...
              (dir == Direction.dRightToLeft && e.Curr.X >= horzLeft))
            {
              ...
              {
                ControlPoint Pt = new ControlPoint(e.Curr.X, horzEdge.Curr.Y);
            ...
            if (e.Curr.X > eNext.Curr.X)
            {
                ...
                if (pt.Y > botY)
                {
                    ...
                    if (Math.Abs(e.Dx) > Math.Abs(eNext.Dx))
                      pt.X = (float)TopX(eNext, botY); else
                      pt.X = (float)TopX(e, botY);
                }
      ...
      private bool IntersectPoint(TEdge edge1, TEdge edge2, out ControlPoint ip)
      {
        ip = new ControlPoint();
        double b1, b2;
        ...
        else
        {
            ...
            if (Math.Abs(edge1.Dx) < Math.Abs(edge2.Dx))
                ip.X = (float)(edge1.Dx * q + b1);
            else
                ip.X = (float)(edge2.Dx * q + b2);
        }
        ...
        if (ip.Y < edge1.Top.Y || ip.Y < edge2.Top.Y)
        {
          if (edge1.Top.Y > edge2.Top.Y)
            ...
            ip.Y = edge2.Top.Y;
          if (Math.Abs(edge1.Dx) < Math.Abs(edge2.Dx))
            ip.X = (float)TopX(edge1, ip.Y);
          else
            ip.X = (float)TopX(edge2, ip.Y);
        }
        return true;
      }
      //------------------------------------------------------------------------------
      ...
      bool JoinHorz(OutPt op1, OutPt op1b, OutPt op2, OutPt op2b, 
        ControlPoint Pt, bool DiscardLeft)
      {
      ...
      private int PointInPolygon(ControlPoint pt, OutPt op)
      {
    ...
    //------------------------------------------------------------------------------
      ...
      private static double DistanceSqrd(ControlPoint pt1, ControlPoint pt2)
      {
      ...
      private static double DistanceFromLineSqrd(ControlPoint pt, ControlPoint ln1, ControlPoint ln2)
      {
      ...
      private static bool SlopesNearCollinear(ControlPoint pt1, 
          ControlPoint pt2, ControlPoint pt3, double distSqrd)
      {
        return DistanceFromLineSqrd(pt2, pt1, pt3) < distSqrd;
      }
      //------------------------------------------------------------------------------

      private static bool PointsAreClose(ControlPoint pt1, ControlPoint pt2, double distSqrd)
      {
          double dx = (double)pt1.X - pt2.X;
          double dy = (double)pt1.Y - pt2.Y;
      ...
      internal static Paths Minkowski(Path poly, Path path, bool IsSum, bool IsClosed)
      {
        ...
        if (IsSum)
          for (int i = 0; i < pathCnt; i++)
          {
            ...
            foreach (ControlPoint ip in poly)
              p.Add(new ControlPoint(path[i].X + ip.X, path[i].Y + ip.Y));
            result.Add(p);
          }
        else
          for (int i = 0; i < pathCnt; i++)
  ...
  public class ClipperOffset
  {
    ...
    private List<ControlPoint> m_normals = new List<ControlPoint>();
    private double m_delta, m_sinA, m_sin, m_cos;
    private double m_miterLim, m_StepsPerRad;

    private ControlPoint m_lowest;
    ...
    public void AddPath(Path path, JoinType joinType, EndType endType)
    {
      ...
      if (m_lowest.X < 0)
        m_lowest = new ControlPoint(0, k);
      else
      {
        ControlPoint ip = m_polyNodes.Childs[(int)m_lowest.X].m_polygon[(int)m_lowest.Y];
        if (newNode.m_polygon[k].Y > ip.Y ||
          (newNode.m_polygon[k].Y == ip.Y &&
          newNode.m_polygon[k].X < ip.X))
          m_lowest = new ControlPoint(m_polyNodes.ChildCount - 1, k);
      }
    }
    //------------------------------------------------------------------------------
    ...
    internal static ControlPoint GetUnitNormal(ControlPoint pt1, ControlPoint pt2)
    {
      double dx = (pt2.X - pt1.X);
      double dy = (pt2.Y - pt1.Y);
      if ((dx == 0) && (dy == 0)) return new ControlPoint();

      double f = 1 / Math.Sqrt(dx * dx + dy * dy);
      dx *= f;
      dy *= f;

      return new ControlPoint(dy, -dx);
    }
    //------------------------------------------------------------------------------
      ...
      for (int i = 0; i < m_polyNodes.ChildCount; i++)
      {
        ...
        if (len == 1)
        {
          if (node.m_jointype == JoinType.jtRound)
          {
            ...
            {
              m_destPoly.Add(new ControlPoint(
                (float)(m_srcPoly[0].X + X * delta),
                (float)(m_srcPoly[0].Y + Y * delta)));
              double X2 = X;
              X = X * m_cos - m_sin * Y;
              Y = X2 * m_sin + Y * m_cos;
            }
        ...
        else if (node.m_endtype == EndType.etClosedLine)
        {
          ...
          m_destPoly = new Path();
          //re-build m_normals ...
          ControlPoint n = m_normals[len - 1];
          for (int j = len - 1; j > 0; j--)
            m_normals[j] = new ControlPoint(-m_normals[j - 1].X, -m_normals[j - 1].Y);
          m_normals[0] = new ControlPoint(-n.X, -n.Y);
          k = 0;
          for (int j = len - 1; j >= 0; j--)
            OffsetPoint(j, ref k, node.m_jointype);
          m_destPolys.Add(m_destPoly);
        ...
        else
        {
          ...
          {
            int j = len - 1;
            pt1 = new ControlPoint((float)(m_srcPoly[j].X + m_normals[j].X *
              delta), (float)(m_srcPoly[j].Y + m_normals[j].Y * delta));
            m_destPoly.Add(pt1);
            pt1 = new ControlPoint((float)(m_srcPoly[j].X - m_normals[j].X *
              delta), (float)(m_srcPoly[j].Y - m_normals[j].Y * delta));
            m_destPoly.Add(pt1);
          ...
          else
          {
            ...
            k = len - 2;
            m_sinA = 0;
            m_normals[j] = new ControlPoint(-m_normals[j].X, -m_normals[j].Y);
          ...
          m_normals[0] = new ControlPoint(-m_normals[1].X, -m_normals[1].Y);
          ...
          if (node.m_endtype == EndType.etOpenButt)
          {
            pt1 = new ControlPoint((float)(m_srcPoly[0].X - m_normals[0].X * delta),
              (float)(m_srcPoly[0].Y - m_normals[0].Y * delta));
            m_destPoly.Add(pt1);
            pt1 = new ControlPoint((float)(m_srcPoly[0].X + m_normals[0].X * delta),
              (float)(m_srcPoly[0].Y + m_normals[0].Y * delta));
            m_destPoly.Add(pt1);
    ...
    public void Execute(ref Paths solution, double delta)
    {
      ...
      {
        RectangleF r = Clipper.GetBounds(m_destPolys);
        Path outer = new Path(4);

        outer.Add(new ControlPoint(r.Left - 10, r.Bottom + 10));
        outer.Add(new ControlPoint(r.Right + 10, r.Bottom + 10));
        outer.Add(new ControlPoint(r.Right + 10, r.Top - 10));
        outer.Add(new ControlPoint(r.Left - 10, r.Top - 10));
    ...
    void OffsetPoint(int j, ref int k, JoinType jointype)
    {
      ...
      if (m_sinA * m_delta < 0)
      {
        m_destPoly.Add(new ControlPoint((float)(m_srcPoly[j].X + m_normals[k].X * m_delta),
          (float)(m_srcPoly[j].Y + m_normals[k].Y * m_delta)));
        m_destPoly.Add(m_srcPoly[j]);
        m_destPoly.Add(new ControlPoint((float)(m_srcPoly[j].X + m_normals[j].X * m_delta),
          (float)(m_srcPoly[j].Y + m_normals[j].Y * m_delta)));
      }
      else
        switch (jointype)
        {
    ...
    internal void DoSquare(int j, int k)
    {
      ...
      m_destPoly.Add(new ControlPoint(
          (float)(m_srcPoly[j].X + m_delta * (m_normals[k].X - m_normals[k].Y * dx)),
          (float)(m_srcPoly[j].Y + m_delta * (m_normals[k].Y + m_normals[k].X * dx))));
      m_destPoly.Add(new ControlPoint(
          (float)(m_srcPoly[j].X + m_delta * (m_normals[j].X + m_normals[j].Y * dx)),
          (float)(m_srcPoly[j].Y + m_delta * (m_normals[j].Y - m_normals[j].X * dx))));
    }
    //------------------------------------------------------------------------------
    ...
    {
      double q = m_delta / r;
      m_destPoly.Add(new ControlPoint((float)(m_srcPoly[j].X + (m_normals[k].X + m_normals[j].X) * q),
          (float)(m_srcPoly[j].Y + (m_normals[k].Y + m_normals[j].Y) * q)));
    }
    //------------------------------------------------------------------------------
    ...
    internal void DoRound(int j, int k)
    {
      ...
      {
        m_destPoly.Add(new ControlPoint((float)(m_srcPoly[j].X + X * m_delta),
            (float)(m_srcPoly[j].Y + Y * m_delta)));
        X2 = X;
        X = X * m_cos - m_sin * Y;
        Y = X2 * m_sin + Y * m_cos;
      }
      m_destPoly.Add(new ControlPoint(
      (float)(m_srcPoly[j].X + m_normals[j].X * m_delta),
      (float)(m_srcPoly[j].Y + m_normals[j].Y * m_delta)));
    }
    //------------------------------------------------------------------------------
  }
